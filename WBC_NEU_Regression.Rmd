...
knitr::opts_chunk$set(echo = TRUE, warning = TRUE, message = TRUE, error = TRUE, comment = "")
...
filtered_data_all_log_mean_cat <- filtered_data_all_log %>%
  ...
  group_by(ID, Degree, Therapy, four_months_OR, four_months_CR) %>%
  summarise(
    log10WBC_mean = mean(log10WBC),
    log10NEU_mean = mean(log10NEU),
    Percent_NEU_to_WBC_mean = mean(Percent_NEU_to_WBC),
    .groups = "drop"
  ) %>%
  mutate(
  ...
  mutate(across(c(log10WBC_mean, log10NEU_mean, Percent_NEU_to_WBC_mean),
    ~ split_by_quantiles(.x, n_categories = 3),
    .names = "{col}_quantiles"
  ))
```
...
filtered_data_all_log_max_cat <- filtered_data_all_log %>%
  ...
  summarise(
    WBC_max = max(WBC),
    NEU_max = max(NEU),
    Percent_NEU_to_WBC_max = max(Percent_NEU_to_WBC),
    .groups = "drop"
  ) %>%
  mutate(
...
```{r}
model <- glm(four_months_OR ~ Percent_NEU_to_WBC_mean_quantiles + Therapy, data = filtered_data_all_log_mean_cat, family = binomial)
summary(model)

model0 <- glm(four_months_OR ~  Therapy, data = filtered_data_all_log_mean_cat, family = binomial)
model1 <- glm(four_months_OR ~ Percent_NEU_to_WBC_mean_quantiles + Therapy, data = filtered_data_all_log_mean_cat, family = binomial)
anova(model0, model1)
```
...
emm <- emmeans(model, ~ Percent_mean_category)
co <- contrast(emm, method = "consec")
tidy(co, conf.int = T) %>%
  ggplot(aes(contrast)) +
  geom_pointrange(aes(y = estimate, ymin = conf.low, ymax = conf.high)) +
  scale_y_continuous(lim = c(-5,5), oob = scales::oob_squish)

# Не очень (разница должна быть примерно одна и та же), даже с учетом широких ДИ

filtered_data_all_log_mean_cat %>%
  group_by(Percent_mean_category) %>%
  summarise(e = sum(four_months_OR == 1),
            n = n())
```
...
```{r}
model <- glm(four_months_OR ~ Percent_NEU_to_WBC_max + Therapy, data = filtered_data_all_log_max_cat, family = binomial)
summary(model)
...
emm <- emmeans(model, ~ Percent_max_category)
co <- contrast(emm, method = "consec")
tidy(co, conf.int = T) %>%
  ggplot(aes(contrast)) +
  geom_pointrange(aes(y = estimate, ymin = conf.low, ymax = conf.high)) +
...
filtered_data_all_log_max_cat %>%
  group_by(Percent_max_category) %>%
  summarise(e = sum(four_months_OR == 1),
            n = n())
```
